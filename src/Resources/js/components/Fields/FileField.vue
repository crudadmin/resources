        ...
        <div class="file-group">
            <div class="upload-file-wrapper">
                <input
                    ...
                    :name="isMultipleUpload ? name + '[]' : name"
                    @change="addFile"
                    class="form-control"
            ...
            <div v-show="isMultiple && !isMultirows && getFiles.length > 0">
                <select
                    ref="multipleFiles"
                    :name="field.hasLocale() || (isMultiple && !isMultirows && getFiles.length > 0) ? '$uploaded_' + name + '[]' : ''"
                    data-placeholder=" "
                    multiple
                >
...
export default {
    ...
    mounted() {
        ...
        eventHub.$on(
            ...
            (this.onSubmitEvent = (data) => {
                ...
                if (data.table != this.model.slug || data.depth_level != this.depth_level) return;
    ...
    computed: {
        ...
        canFileBeDownloaded() {
            return (
                this.value &&
                !this.hasMultipleFilesValue &&
                this.file_from_server &&
                !this.isMultiple &&
    ...
    methods: {
        ...
        addMultipleFilesSupport(onInitialize) {
            ...
            if (this.field.type == 'file' && this.isMultiple && !this.isMultirows) {
                let chosen = $(this.$refs.multipleFiles)
                    .chosen({
            ...
            if (onInitialize == true) {
                ...
                $(this.$refs.multipleFiles)
                    .parent()
                    .on('click', 'li.search-choice span', (e) => {
