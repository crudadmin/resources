    ...
    <Field :field="field" :class="{ disabled: disabled || readonly || hasNoFilterValues }" v-show="required || !hasNoFilterValues">
        <div class="form-group__chosen-container" :class="{ canPerformActions: isEnabledRelationModal }">
            <select
                ...
                :data-placeholder="field.placeholder ? field.placeholder : trans('select-option-multi')"
                :multiple="isMultiple"
                class="form-control"
            >
                <option v-if="!isMultiple" value="">{{ trans('select-option') }}</option>
                <option v-for="mvalue in missingValueInSelectOptions" :key="mvalue" :value="mvalue" :selected="hasValue(mvalue, value, isMultiple)">
                    {{ mvalue }}
                </option>
                <option
                    ...
                    :value="data[0]"
                >
                    {{ data[1] == null ? trans('number') + ' ' + data[0] : data[1] }}
                </option>
            </select>
            ...
            <AddNewRowModal :id="id" :model="relationModel" v-slot="{ open }" v-if="isEnabledRelationModal">
                <div class="button-actions">
                    <button
                        ...
                        @click="performRelationAction('add', open)"
                        type="button"
                    ...
                    <button
                        v-if="canEditRow"
                        data-edit-relation-row
                        @click="performRelationAction('edit', open)"
                        type="button"
                    ...
                    <button
                        v-else-if="canViewRow"
                        data-view-relation-row
                        @click="performRelationAction('view', open)"
                        type="button"
                        class="action-button btn btn-default"
                    >
                    ...
                    <button
                        v-if="canListRow"
                        data-add-relation-row
                        @click="performRelationAction('list', open)"
                        type="button"
                        class="action-button btn btn-default"
                    >
...
export default {
    ...
    computed: {
        ...
        canListRow() {
            return this.field.canList && this.hasRelationAccess('read') && (!this.getFilterBy || this.filterByValue);
        },
        getFilterBy() {
        ...
        fieldOptions() {
            if (typeof this.field.options != 'object') return [];
        ...
        missingValueInSelectOptions() {
            ...
            var options = this.fieldOptions,
                missing = [],
                originalValue = this.$parent.getLocalizedValue(!_.isNil(this.field.$originalValue) ? this.field.$originalValue : this.field.value);
    ...
    methods: {
        ...
        getFilter(options) {
            ...
            if (this.getFilterBy) filter[this.getFilterBy[1]] = this.isStaticFilterColumn ? this.getStaticFilterBy : this.filterByValue;
