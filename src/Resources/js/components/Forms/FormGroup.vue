<template>
    <div
        ...
        class="fields-group fields-group__item"
        :group-id="group.id"
        :class="getGroupClass(group)"
        :data-fields="visibleFields.length"
    >
        <div
            :class="{
                card: canShowGroupName(group),
                'fields-group__wrapper': visibleFields.length > 0 && hasTabs(group.fields) === false,
                'fields-tabs__wrapper': hasTabs(group.fields),
            }"
        >
            ...
            <div :class="{ 'card-body': canShowGroupName(group) }">
                <div class="row">
                    <div v-if="hasTabs(group.fields)" class="col-lg-12 tab--parent">
                        <form-tabs-builder :level="level + '-' + group.id" :tabs="tabsFields" :model="model"></form-tabs-builder>
                    </div>
                    ...
                    <fragment :key="index" v-for="(item, index) in group.fields">
                        ...
                        <form-group v-if="isGroup(item) && !isTab(item)" :level="addGroupLevel(level)" :group="item" :model="model"></form-group>
                    </fragment>
...
export default {
    ...
    methods: {
        ...
        getVisibleFields(fields) {
            return (fields || []).filter((item) => {
                if (typeof item == 'string') {
                    ...
                    return !(this.model.tryAttribute(field, 'invisible') || this.canRemoveAttribute(field) || !this.canShowField(field));
                }
        ...
        canRemoveAttribute(field) {
            return (
                this.model.tryAttribute(field, 'removeField') ||
        ...
        canHideAttribute(field) {
            ...
            return this.model.tryAttribute(field, 'hideField') || this.model.tryAttribute(field, 'hideFromForm') || visibleField === false;
        },
        canRenderField(field) {
            return !this.model.tryAttribute(field, 'invisible') && !this.canRemoveAttribute(field);
        },
        canShowField(field) {
            if (this.canHideAttribute(field)) return false;

            if ((field.ifExists === true || field.hideOnCreate === true) && !this.model.isOpenedRow()) return false;

            if ((field.ifDoesntExists === true || field.hideOnUpdate === true) && this.model.isOpenedRow()) return false;
        ...
        getGroupClass(group) {
            ...
            if (width[0] == 'half') width[0] = 6;
            else if (width[0] == 'full') width[0] = 12;
            else if (width[0] == 'third') width[0] = 4;
        ...
        isGroupVisible(group) {
            if (group.attributes && Object.keys(group.attributes).length > 0) {
                if (this.model.tryAttribute(group.attributes, 'hideField') || this.model.tryAttribute(group.attributes, 'hideFromForm')) {
                    return false;
                }
