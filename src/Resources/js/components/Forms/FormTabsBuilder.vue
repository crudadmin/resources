<template>
    <div :class="{ 'nav-tabs-custom': hasTabsAvailable }">
        ...
        <div
            class="tab-content tab-content--form"
            :class="{ '--model-active': isModel(getTabs[activeTab]) && !getTabModel(getTabs[activeTab]).inParent }"
...
export default {
    ...
    computed: {
        ...
        hasTabsAvailable() {
            return !(
                ...
                ).length == 1 && this.getTabs[0].default === true
            );
        },
    },
    ...
    methods: {
        ...
        canRenderTab(tab, index) {
            ...
            if (this.hasTabs(tab.fields) || this.isModel(tab)) return true;
         ...
         */
        getTabIcon(tab) {
            if (this.isModel(tab)) return tab.icon || this.getTabModel(tab).icon;
         ...
         */
        isModel(tab) {
            if (!(tab.type == 'tab' && tab.model && this.getTabModel(tab).active == true)) return false;
        ...
        chunkGroups(fields) {
            ...
            for (var i = 0; i < fields.length; i++) {
                ...
                if (this.isField(fields[i])) data[data.length - 1].push(fields[i]);
                else {
                    data.push(fields[i]);
                }
            }
