    ...
    input.parents('.tab-pane').each(function () {
        ...
        getActiveTab($(this)).each(function () {
            ...
            $(this)
                ...
                .attr('data-original-title', $app.trans('tab-error'))
                .attr('has-error', '')
                .one('click', function () {
                ...
                .find('a')
                .prepend('<i class="nav-link--icon-left fa fa-exclamation-triangle"></i>');
        });
    });
};
...
const showErrorMessage = (model, message, i) => {
    ...
    return function () {
        ...
        if ($(this).is('select')) {
            ...
            if (where.find('.help-block').length == 0) where = where.children().last().prev();
            else where = null;
...
var FormActions = (Model) => {
    ...
    Model.prototype.getSelectedLanguageId = function () {
        ...
        return this.getData('selected_language_id') ? this.getData('selected_language_id') : this.getData('langid');
    };
    ...
    Model.prototype.getFormTitle = function (e) {
        ...
        //Insert title
        else if ((title = this.getSettings('title.insert')) || (title = this.getSettings('title.create'))) {
            return title;
        }
