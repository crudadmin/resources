...
const updateFieldOptions = (model, fields, requestModel) => {
    ...
    for (var key in fields) {
        ...
        if (
            ...
            (typeof fields[key].options === 'string' || Object.keys(fields[key].options).length > 0)
        ) {
...
const isNumericValue = (model, key) => {
    if ([model.getKeyName(), '_order'].indexOf(key) > -1) return true;
...
var ModelTableRows = (Model) => {
    ...
    Model.prototype.checkFormVisibility = function () {
        if (
            ((!this.isOpenedRow() && !this.canAddRow()) || (this.isOpenedRow() && this.editable == false && this.displayable !== true)) &&
            !this.isInParent()
        ) {
            return false;
        }
    ...
    Model.prototype.checkActiveGridSize = function (columns) {
         ...
         */
        if (hasModelsInGroups(this.fields_groups) || this.hasChilds() > 0 || columns.length > fullSizeByColumns()) {
            return (sizes[3].active = true);
        }
    ...
    Model.prototype.loadRows = async function (options = {}) {
        const { indicator = true, download = false, page = null, limit = null, update = false, query = {} } = options || {};
    ...
    Model.prototype.getRows = function (options) {
        ...
        return rows.sort((a, b) => {
            ...
            (a = getEncodedValue(aValue, is_decoded)), (b = getEncodedValue(bValue, is_decoded));
