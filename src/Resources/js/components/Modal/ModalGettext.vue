<template>
    <Modal :modal="modal" :options="{ class: 'gettext-table', actions: actions }">
        ...
        <p v-if="!loaded" class="loading">
            <i class="fa fa-refresh fa-spin"></i>
            {{ trans('gettext-loading') }}
        </p>
        ...
        <div class="translation-actions">
            <a @click="showMissing = true" v-if="missing.length > 0 && showMissing == false">
                {{ trans('gettext-showmissing') }} ({{ missing.length }})
            </a>
...
export default {
        ...
        filtratedTranslates() {
            ...
            for (var key in this.availableTranslated) {
                ...
                if (
                    ...
                    (!this.query ||
                        this.hasChange(key) ||
                        this.availableTranslated[key].join('').toLowerCase().indexOf(query) > -1 ||
                        finalKey.toLowerCase().indexOf(query) > -1)
                ) {
    ...
    methods: {
        ...
        async loadTranslations() {
            var url = this.$root.requests.translations.replace(':id', this.row.id).replace(':table', this.model.table);
