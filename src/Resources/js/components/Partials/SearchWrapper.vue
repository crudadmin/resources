<template>
    <div class="searchbar__wrapper" :class="{ '--hasMoreButton': model.getData('searching') }" v-if="model.canShowSearchBar()">
        <div class="searchbar__wrapper__queries">
            <search v-for="(query, $index) in search.queries" :key="$index" :search="query" :model="model"></search>
        </div>
        <button
            data-toggle="tooltip"
...
export default {
    ...
    methods: {
        addSearchQuery() {
            this.model.getData('search').queries.push(_.cloneDeep(defaultSearchQuery));
        },
    },
};
</script>
