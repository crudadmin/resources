<template>
    <select
        :data-placeholder="placeholder || (multiple ? trans('select-option-multi') : trans('select-option'))"
        :multiple="multiple"
        :disabled="disabled"
    >
...
export default {
    ...
    computed: {
        localOptions() {
            ...
            return this.allowEmpty ? [{ [this.trackBy]: null, [this.label]: '' }].concat(options) : options;
        },
        localValue() {
