<template>
    <div>
        ...
        <div
            ...
            :class="{ 'admin-model--single-mode': model.isSingle(), 'admin-model--in-parent': model.isInParent() }"
            v-show="model.canShowForm() || model.canShowRows() || isSearching"
        >
            <div class="admin-model__header" :class="{ 'with-border': model.isSingle() }" v-show="canShowAdminHeader">
                <div class="left">
                    ...
                    <RowsFilter
                        :model="model"
                        v-if="model.isSettingEnabled('filter', true) && (!model.isEnabledOnlyFormOrTableMode() || !model.canShowForm())"
                    />
                </div>
                ...
                <div class="right" v-if="!model.isSingle()">
                    ...
                    <div
                        class="dropdown"
                        v-if="hasLanguages && isActiveLanguageSwitch && (!model.canShowForm() || model.activeGridSize() != 0)"
                        data-global-language-switch
                    >
                        ...
                        <ul class="dropdown-menu dropdown-menu-right">
                            <li v-for="language in languages" :class="{ active: langid == language.id }" @click="changeLanguage(language.id)">
            ...
            <div class="admin-model__body">
                <div class="row grid-table" :class="{ 'grid-fullsize': model.activeGridSize() == 0 }">
                    <!-- left column -->
                    <div
                        class="col--rows col-12"
                        :class="['col-lg-' + (12 - (12 - model.activeGridSize())), { '--noMargin': !model.canShowForm() }]"
                        v-show="canDisplayRowsLayout"
                    >
                        <custom-components :model="model" type="table-before" />

                        <model-rows-builder v-show="model.isSettingEnabled('table', true)" :model="model" :rows="rows"></model-rows-builder>
                    ...
                    <div :class="['col-lg-' + (12 - model.activeGridSize())]" class="col--form col-12" v-show="model.canShowForm()">
                        ...
                        <form-builder v-if="model.isSettingEnabled('form', true)" :model="model"></form-builder>
...
export default {
                ...
                .setData('sizes', [
                    {
                        ...
                        active: false,
                        disabled: this.model_builder.isSettingDisabled('grid.small'),
                    },
                    { size: 8, key: 'big', name: this.trans('grid-big'), active: false, disabled: this.model_builder.isSettingDisabled('grid.big') },
                    {
                        ...
                        active: false,
                        disabled: this.model_builder.isSettingDisabled('grid.medium'),
                    },
                    {
                        size: 0,
                        key: 'full',
                        ...
                        active: false,
                        disabled: this.model_builder.isSettingDisabled('grid.full'),
                    },
                ])
                .getData('sizes'),
    ...
    methods: {
        ...mapMutations('models', ['storeModel', 'removeModel']),
        onModelUpdate() {
    ...
    computed: {
        ...
        canShowAdminHeader() {
            ...
            return (
                ...
                (this.ischild && !this.model.isSingle()) ||
                //For parent models, we can display header only if
                (!this.model.isSingle() && (this.model.isEnabledGrid() || this.model.canShowSearchBar() || this.canShowAddButton))
            );
        },
        ...
        canShowAddButton() {
            ...
            return (
                this.model.canAddRow() &&
                !this.model.isSingle() &&
                this.model.hasAccess('insert') &&
                !this.model.isOnlyFormOpened() &&
                this.model.isSettingEnabled('buttons.create', true)
            );
        },
        isSearching() {
        ...
        isLocaleModel() {
            ...
            for (var key in this.model.fields) if (this.model.fields[key].locale == true) return true;
