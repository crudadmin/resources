<?php

return [
    'admin'     => 'Administrácia',
    'logout'    => 'Odhlásiť sa',
    'welcome'   => 'Vitajte',
    'welcome-in-admin'   => 'Vitajte v administrácii',

    'auth-disabled' => 'Váš účet bol deaktivovaný.',
    'admin-user'   => 'Administrátor',
    'auto-logout'   => 'Boli ste automatický odhlásený. Prosím, znova sa prihláste.',
    'unknown-error' => 'Nastala nečakaná chyba, skúste neskôr prosím.',

    'navigation' => 'Navigácia',
    'language-mutation' => 'Jazyková verzia',
    'language-title' => 'Upravte si nastavenia jazykových mutácii. Taktiež si nastavte predvolený jazyk umiestnením jazyka na prvú pozíciu.',
    'languages-add-new' => 'Pridať nový jazyk',
    'languages-update' => 'Upravujete jazyk :name',
    'languages-name' => 'Názov jazyka',
    'languages-title' => 'Zadajte názov jazyka',
    'languages-prefix' => 'Skratka jazyka',
    'languages-prefix-title' => 'Zadajte skratku jazyka (en, sk, de, cz)...',
    'languages-po-name' => 'Preklad .po súboru',
    'languages-mo-name' => 'Preklad .mo súboru',
    'languages' => 'Jazyková mutácia',
    'languages-missing' => 'V administrácii sa nenachádzajú žiadne jazykové mutácie, ku ktorým by mohli byť záznamy naviazané. Prosím, pridajte aspoň jeden jazyk.',
    'languages-field' => 'Tento vstup je prekladaný do viacerých jazykov.',
    'change-language' => 'Zmena jazykovej mutácie. Pozor! Zmena jazyka môže zahodiť Vaše aktuálne rozpísane zmeny.',

    'super-admin' => 'Super administrátor',
    'admin-group' => 'Administrátorská skupina',
    'user-groups' => 'Používateľské skupiny',
    'user-groups-title' => 'Upravte používateľské skupiny a ich obmedzenie, do ktorých budú následne priradený useristrátori',
    'user-groups-name' => 'Názov skupiny',
    'user-groups-placeholder' => 'Zadajte názov skupiny',
    'user-groups-modules' => 'Povolené moduly',

    'roles-module' => 'Modul',
    'roles-read' => 'Čítanie',
    'roles-insert' => 'Vkladanie',
    'roles-update' => 'Úprava',
    'roles-publishable' => 'Publikácia',
    'roles-delete' => 'Mazanie',
    'roles-all' => 'Všetko',

    'seoroutes-url' => 'Url adresa',
    'seoroutes-newurl' => 'Nová url adresa',
    'seoroutes-group' => 'Skupina',
    'seoroutes-title' => 'Titulok stránky',
    'seoroutes-keywords' => 'Kľúčové slova',
    'seoroutes-description' => 'Popis stránky',
    'seoroutes-images' => 'Obrázky stránky',

    'check-all' => 'Označiť všetky',
    'uncheck-all' => 'Odznačiť všetky',
    'show' => 'Zobraziť',
    'show-image' => 'Zobraziť obrázok',
    'download' => 'Stiahnuť',
    'download-document' => 'Stiahnuť dokument',
    'download-file' => 'Stiahnuť súbor',

    'history.changes' => 'História zmien',
    'history.who' => 'Zmeny vykonal',
    'history.count' => 'Počet zmien',
    'history.date' => 'Dátum zmeny',
    'history.system' => 'Systém',
    'history.show' => 'Zobraziť zmeny',

    'close' => 'Zatvoriť',
    'accept' => 'Potvrdiť',
    'loading' => 'Načítavam dáta...',

    'created' => 'Vytvorené',
    'created-at' => 'Vytvorené dňa',
    'last-change' => 'Posledná zmena',
    'published-at' => 'Publikované dňa',

    'info' => 'Informácia',
    'warning' => 'Upozornenie',
    'row-info' => 'Informácie o zázname',
    'row-info-n' => 'Informácie o zázname č.',
    'no-changes' => 'V aktuálnom zázname neboli vykonané žiadne zmeny.',
    'delete-warning' => 'Naozaj chcete vymazať daný záznam?',

    'cannot-create' => 'Nie je možné pridať nový záznam.',
    'cannot-edit' => 'Tento záznam nie je možné upravovať.',
    'cannot-delete' => 'Tento záznam nie je možné vymazať.',
    'cannot-delete-multiple' => 'Jeden zo záznamov nie je možné vymazať.',
    'cannot-publish' => 'Tento záznam nie je možné znepublikovať.',
    'cannot-sort' => 'Tento záznam nie je možné presúvať.',
    'success-created' => 'Záznam bol úspešne pridaný',
    'success-save' => 'Záznam bol úspešne uložený',
    'with-errors' => 's nasledujúcimi chybami',

    'multilanguages' => 'Tento záznam je viacjazyčný',
    'rows' => 'Záznamy',
    'saving' => 'Ukladá sa',
    'save' => 'Uložiť',
    'send' => 'Odoslať',
    'sending' => 'Odosiela sa',
    'edit-row-n' => 'Upraviť záznam č.',
    'new-row' => 'Nový záznam',
    'general-tab' => 'Všeobecné',
    'number' => 'č.',
    'rows-list' => 'Zoznam stĺpcov',
    'default' => 'Predvolené',

    'edit' => 'Upraviť',
    'show' => 'Zobraziť',
    'hide' => 'Skryť',
    'action' => 'Akcia',
    'publish-toggle' => 'Publikovať/Skryť',
    'delete' => 'Vymazať',
    'delete-file' => 'Vymazať súbor',
    'select-option' => 'Vyberte jednu z možností',
    'select-option-multi' => 'Vyberte zo zoznamu možností',
    'no-option' => 'Žiadna možnosť',

    'edit-size' => 'Upravte šírku zobrazenia formulára pre prehľadnejšie zobrazenie záznamov',
    'search' => 'Vyhľadajte',
    'search-all' => 'Hľadať všade',
    'get-value' => 'Vyberte hodnotu',
    'reset' => 'Resetovať',
    'on' => 'Vypnuté',
    'off' => 'Zapnuté',
    'yes' => 'Áno',
    'no' => 'Nie',

    'rows-count' => 'Počet záznamov v tabuľke',
    'show-all' => 'Zobraziť všetko',
    'confirmation' => 'overenie znova',

    'success-save' => 'Zmeny boli úspešne uložené',
    'no-permissions' => 'Nemáte právomoc k pristúpeniu do tejto sekcie.',
    'migrate-error' => 'Nastala nečakaná chyba, pravdepodobne ste nespustili migráciu modelov pomocou príkazu:',
    'db-error' => 'Nastala nečakaná chyba. Váš administrátor pravdepodobne zabudol aktualizovať databázu. Ak táto chyba pretrváva, kontaktujte ho.',
    'row-error' => 'Záznam neexistuje, pravdepodobne už bol vymazaný.',
    'component-missing' => 'Komponenta s názvom <strong>%s</strong> pre vstup <strong>%s</strong> nebol nájdený.',
    'lang-error' => 'Hodnoty predvoleného jazyka sú povinné. Prosíme, skontrolujte vyplnené hodnoty.',

    'login' => 'Prihlásiť sa',
    'password' => 'Heslo',
    'password-again' => 'Heslo znova',
    'login-with' => 'Prosím, prihláste sa pomocou emailu a hesla',
    'remember-password' => 'Zapamätať si',
    'forgot-password' => 'Zabudol som heslo',
    'send-password' => 'Odoslať potvrdzovací link na email',
    'password-reset' => 'Žiadosť o reset hesla',
    'password-reset-button' => 'Resetovať heslo',
    'tab-error' => 'Záložka nemá vyplnené povinné údaje.',

    'gettext-open' => 'Upraviť jazykové mutácie',
    'gettext-update' => 'Upraviť preklady',
    'gettext-search' => 'Vyhľadajte pomocou výrazu',
    'gettext-no-match' => 'V prekladoch nebola nájdená žiadna zhoda.',
    'gettext-source' => 'Základný text',
    'gettext-translation' => 'Preklad',
    'gettext-count' => 'Zobraziť všetky preklady',
    'gettext-showmissing' => 'Zobraziť odstránené preklady',
    'gettext-missing' => 'Tento preklad bol odstránený, v aplikácii sa už nevyužíva.',
    'gettext-loading' => 'Preklady sa načitávajú, prosím počkajte.',
    'gettext-save' => 'Uložiť a zatvoriť',

    'grid-small' => 'Veľky formulár',
    'grid-big' => 'Veľká tabuľka',
    'grid-medium' => 'Vedľa seba',
    'grid-full' => 'Plná šírka',
];
